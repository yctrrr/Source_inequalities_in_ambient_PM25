...
PM_source_risk_China <- fread("result/Interpretation/SHAP_China_TAP/PM_Source_Health_Risk_2001-2019_TAP_China.csv")
PM_source_risk_China%>%
  filter(Year%in%c(2000,2005,2010,2015,2017,2019))%>%
  group_by(Year)%>%
  ...
  ungroup()%>%
  mutate_at(c(paste0("PM25_",sector)), ~ 100*.x/anthro_sum)%>%
  as.data.frame()
...
# data = County_join_China
# data_sf = County_sf_China
...
decomp_v <- c("PM25","PM25_risk",paste0("PM25_",sector_v),paste0("PM25_",sector_v,"_risk"))
# decomp_v <- c("PM25",paste0("PM25_",sector_v))

## extract the county-level source-specific PM2.5 and associated risks
PM_source_risk_county_China <- 2001:2019%>%
  map_dfr(~ PM_source_risk_China%>%
            filter(Year == .x)%>%
...
EMI_source_county_China <- 2001:2019%>%
  map_dfr(\(year){
    sector_v%>%
      # purrr::map(~ EMI_data_read(sector = .x, year = year, Inv_name = "CEDS"))%>%
...
# fwrite(EMI_source_county_China)
...
is.pbalanced(County_rejoin_China%>%filter(nb_ind == 0), index = c("CountyID","Year"))
County_rejoin_China%>%summary()

fwrite(County_rejoin_China, "result/Inequality/Economic_imapct_county_dataset_China.csv")
...
# decomp_v <- c("PM25","PM25_risk",paste0("PM25_",sector),paste0("PM25_",sector,"_risk"))
# decompvar <- c("EMI","MET","PM2.5_MYA","baseline","resid",metvar,sector)
...
Grid_join_cal_China <- function(year){
  ...
  Pop <- rast(Pop_file)%>%
    terra::resample(LU, method = "sum") ## resample on 0.1g
  ...
  Grid_GDP_pc%>%
    ...
    # dplyr::select(X_Lon,Y_Lat,GridID,GDP_USD)%>%rast%>%plot
}

Grid_join_China <- 2001:2019%>%
  map_dfr(~ Grid_join_cal_China(.x))
Grid_join_China%>%names

# PM_source_risk_China <- PM_risk_China
Grid_join_China <- fread(paste0("result/Inequality/Economic_imapct_grid_dataset_China.csv"))%>%
  rename(Pop_all = Pop)%>%
  dplyr::select(X_Lon,Y_Lat,GridID,Year,all_of(iv_china))

Grid_rejoin_China <- Grid_join_China%>%
  rename(Pop = Pop_all)%>%
  left_join(Grid_info_China,.)%>%na.omit%>%
  Grid_data_label()%>%
  left_join(PM_source_risk_China%>%dplyr::select(-Pop))
  # left_join(PM_source_extend_China)

# PM_source_annual_China%>%
#   filter(Year == 2000)%>%
#   dplyr::select(X_Lon,Y_Lat,PM25)%>%
#   rast%>%plot
# Grid_join_China%>%
#   filter(Year == 2000)%>%
#   dplyr::select(X_Lon,Y_Lat,GridID,GDP_USD)%>%rast%>%plot
Grid_rejoin_China%>%
  filter(Year == 2001)%>%
  dplyr::select(X_Lon,Y_Lat,GridID,GDP_USD,PM25_industry)%>%
  rast%>%plot
...
County_sr_SY_divide <- County_rejoin_China%>%
  ...
  dplyr::select(-GDP_USD)%>%
  rename(GDPQ = GDP_USD_group)
fwrite(County_SY_divide, paste0("result/Inequality/County_single_year_division_CEC_area_2001-2019.csv"))
...
County_sf_rejoin_China <- fread("result/Inequality/Economic_imapct_county_dataset_China.csv", encoding = 'UTF-8')%>%
  # mutate(PM25 = EMI+MET+baseline+resid+PM25_MYA)%>%
  mutate_at(all_of(paste0("PM25_",sector)), ~ ifelse(.x<0,0,.x))%>%
  left_join(County_sf_China%>%dplyr::select(CountyID),.)%>%na.omit()

Grid_rejoin_China <- fread(paste0("result/Inequality/Economic_imapct_grid_dataset_China.csv"))
Grid_rejoin_China

County_sf_rejoin_China%>%
  st_drop_geometry()%>%
  group_by(Year)%>%
  summarise(across(c(paste0("PM25_",sector),"PM25"), ~ weighted.mean(.x, Pop)))%>%
  ungroup()%>%
  mutate_at(paste0("PM25_",sector), ~.x/PM25*100)%>%
  mutate(Ssum = PM25_agriculture+PM25_industry+PM25_power+PM25_residential+PM25_transportation)%>%
  as.data.frame()
...
##======Visualization (Spatial distribution of GDP rank in two countries)======
source('D:/shaoyanchuan/codebook/function/Sf_visualization.R')
County_sf_join_USA <- fread("result/Inequality/Economic_imapct_model_dataset_USA.csv")%>%
  left_join(County_sf_USA%>%dplyr::select(CountyID),.)%>%na.omit()

County_sf_join_China <- fread("result/Inequality/Economic_imapct_model_dataset_China.csv")%>%
  left_join(County_sf_China%>%dplyr::select(CountyID),.)%>%na.omit()

GDP_rank_plot_set <- function(base_plot, colors = rev(RColorBrewer::brewer.pal(7,"YlGnBu"))){
  base_plot +
    ...
    theme(legend.key.width=unit(2, "cm"),
          ...
          legend.box.background = element_rect(size= 0.5,colour = "black")) ##legend text size
}

County_GDPrank_China_plot <- County_sf_join_China%>%
  filter(Year == 2001)%>%
  sf_polygon_plot(value = "GDPQ")%>%
  GDP_rank_plot_set()
CairoPNG(paste0("result/Inequality/GDP_Quantile_Rank_China.png"),height = 1700,width = 2800, res = 200)
print(County_GDPrank_China_plot)
dev.off()

County_GDPrank_USA_plot <- County_sf_join_USA%>%
  sf_polygon_plot(value = "GDPQ")%>%
  GDP_rank_plot_set()
CairoPNG(paste0("result/Inequality/GDP_Quantile_Rank_USA.png"),height = 1700,width = 2800, res = 200)
print(County_GDPrank_USA_plot)
dev.off()
