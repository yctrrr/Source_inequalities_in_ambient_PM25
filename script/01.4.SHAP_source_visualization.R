...
PM_source_deviation_China <- list(PM_source_long_SHAP_China, PM_source_long_GC_China,
  ...
  mutate(PM25_compare = ifelse(Year == 2017, PM25_GBD, PM25_GC),
         ...
         PM25_SHAP_ratio = PM25_SHAP/PM25, PM25_compare_ratio = PM25_compare/PM25,
...
## test here
PM_source_ratio_GBD_2017%>%
  mutate(Ant_con = rowSums(dplyr::select(.,6:10)))%>%
  mutate(Ant_con = ifelse(Ant_con > 0.9, 0.9, Ant_con))%>%
  dplyr::select(1:2,Ant_con)%>%rast%>%plot

PM_source_extend_China%>%
  filter(Year == 2017)%>%
  # filter(ProvID %in% 410000)%>%
  mutate_at(paste0("PM25_",sector), ~ ifelse(.x < 0, 0, .x/PM25))%>%
  mutate(Ant_con = rowSums(dplyr::select(.,5:9)))%>%
  mutate(Ant_con = ifelse(Ant_con > 0.9, 0.9, Ant_con))%>%
...
labels <- label_gen(breaks)
year_seq <- c(2001,2013,2019)
# colors <- rev(colorspace::sequential_hcl(length(labels), palette = "YlOrRd"))
# colors <- rev(colorspace::diverge_hcl(length(labels), palette = "Blue-Red"))
# colors <- rev(RColorBrewer::brewer.pal(length(labels), "RdYlGn"))
...
year = 2019
# breaks <- c(-Inf,1,3,5,10,15,20,+Inf)
...
PW_source_PM_by_province <- fread(paste0("result/Interpretation/Population-weighted_souce_specific_PM2.5_by_province_China.csv"))%>%
  ...
  filter(Sector!="PM25")%>%
  mutate(Sector = gsub("PM25_","",Sector)%>%str_to_title())
# Source_ineq_ts_vis(PM_source_PW, x = "Year", y = "PM25", source = "Sector",
#                    facet = NULL, title = expression(paste("PM"[2.5]," concentration (μg/m"^"3",")")), colors = NULL)
...
##======Health burden======
PM_risk_China <- fread("result/Interpretation/SHAP_China_TAP/PM_Source_Health_Risk_2001-2019_TAP_China.csv")
for(year in c(2001,2013,2017,2019)){
  PM_risk_apportion_China <- PM_risk_China%>%
    filter(Year == year)%>%dplyr::select(-GridID)%>%
    dplyr::select(X_Lon, Y_Lat, paste0("PM25_",sector,"_risk"))%>%
    dfproj(toCRS = CRS_China, method = "sum")%>%rename(X_Lon = x, Y_Lat = y)%>%
    pivot_longer(paste0("PM25_",sector,"_risk"), names_to = "Sector")%>%
    mutate(Sector = gsub("*PM25_|_risk","",Sector)%>%str_to_title())
  
  breaks <- c(-Inf,2.5,5,10,15,25,40,50,100,150,300,+Inf)
  labels <- label_gen(breaks)
  colors <- rev(RColorBrewer::brewer.pal(length(labels),"Spectral"))
  
  PM_riskmod_China <- PM_risk_apportion_China%>%
    mutate_at(c("value"), ~ cut(.,breaks,labels))

  PM_riskmod_China_plot <- PM_riskmod_China%>%
    grid_facet_plot(x = "X_Lon", y = "Y_Lat", value = "value",
                    facet = "Sector", facet_row = 2, facet_text = 30)%>%
    grid_plot_set(area = "China", colors = colors,
                  title = expression(paste("PM"[2.5],"-related deaths (#/ 100km"^"2",")")),
                  setting = "facet")

  ggplot_oupt(PM_riskmod_China_plot,filename = paste0("result/Source_change/PM25_source_health_burden_",year,".png"),
              export_type = "Cairo", plot_type = "facet")
}
...
Grid_rejoin_China%>%
  ...
  # mutate_at(c(paste0("PM25_",sector),"PM25"), ~ 100*.x/anthro_sum)%>%
  ungroup()%>%
  as.data.frame()
...
# breaks <- c(-Inf,-50,-25,-15,-5,5,15,25,50,+Inf)
...
# labels <- label_gen(breaks)
# colors <- rev(colorspace::sequential_hcl(length(labels), palette = "Purple-Blue"))
# PM_samod_GC_rel_diff_plot <- PM_source_GC_rel_diff%>%
#   Abs_source_plot(2019, breaks, colors, "China",
#                   title = expression(paste("Absolute errors of contribution ratios (%)")))
# ggplot_oupt(PM_samod_GC_rel_diff_plot, filename = paste0("result/Source_change/China/PM25_source_contributions_relative_diff_GC_2019.png"),
#             export_type = "Cairo", plot_type = "facet")
...
Grid_add_info <- fread(paste0(dat_dir,"Shp/GS(2019)1822/Grid_Additional_Information_0.1g.csv"))
...
## calculate population-weighted source-specific PM2.5 in 2001 and 2019
PM_source_extend_China%>%
  filter(Year%in%c(2001,2019))%>%
  mutate_at(paste0("PM25_",sector), ~ifelse(.x < 0,0,.x))%>%
  list(Pop_SY, Grid_add_info)%>%
  reduce(left_join)%>%
  mutate(RegionS = ifelse(Region != "BTH","Other","BTH"))%>%
  group_by(Year,RegionS)%>%
  summarise_at(paste0("PM25_",sector), ~weighted.mean(.x, Pop))%>%
